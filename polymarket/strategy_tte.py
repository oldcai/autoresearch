...
import os
EDGE_TTE_REF = float(os.environ.get('EDGE_TTE_REF', '1e12'))   # require edge ∝ tte above this ref (minutes); 1e12 = off (= baseline)
...
class Strategy:
    ...
    def on_step(self, obs):
        ...
        for j in range(len(obs.idx)):
            ...
            thr = EDGE_THRESHOLD * max(1.0, obs.tte_min[j] / EDGE_TTE_REF)   # 周期越长要求 edge 越高(年化 edge 一致,资金周转)
            if edge > thr and p_mkt >= FAVORITE_MIN:
                usd = scale * min(ORDER_USD * edge / EDGE_THRESHOLD, 4 * ORDER_USD)
                orders.append((int(obs.idx[j]), 'BUY_YES', usd,
                               min(p_mkt + 0.01, 0.99)))
            elif -edge > thr and p_mkt <= 1 - FAVORITE_MIN:
                usd = scale * min(ORDER_USD * -edge / EDGE_THRESHOLD, 4 * ORDER_USD)
